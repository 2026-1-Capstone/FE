...
const Review = () => {
  ...
  useEffect(() => {
    ...
    if (videoId) {
      const targetApiUrl = `${BACKEND_BASE_URL}/api/blur/download/${videoId}`;
      ...
      .then((response) => {
        ...
        console.log("[CORS 우회 성공] 영상 파일 메모리 로드 완료:", blobUrl);
      })
      .catch((err) => {
        console.error("가공 영상 바이너리 획득 실패:", err);
        setLoadingVideo(false);
      });
    }
  }, [navigate]);
      ...
      <div className="review-container">
        ...
        <div className="review-content">
          <div className="preview_div">
            ...
            <div className="video">
              {loadingVideo ? (
                <div className="video-placeholder">비디오 스트림 보안 가드를 해제하며 영상을 로드 중입니다...</div>
              ) : videoUrl ? (
                <video 
                  src={videoUrl} 
                  controls 
                  autoPlay 
                  style={{ width: "100%", height: "100%", objectFit: "contain", borderRadius: "12px", backgroundColor: "#000" }}
                />
              ) : (
                <div className="video-placeholder">영상을 불러올 수 없습니다.</div>
              )}
            </div>
          </div>
            ...
            <div className="info_contents">
              ...
              <div className="info_normal_div">
                ...
                <div className="info_text">
                  {videoInfo ? `${videoInfo.duration} (${videoInfo.size})` : "-"}
                </div>
              </div>
              <div className="info_normal_div">
                <div className="info_title">해상도</div>
                <div className="info_text">자동 맞춤 (1280p 가공)</div>
              </div>
              <div className="info_normal_div">
              ...
              <div className="info_normal_div">
                ...
                <div className="info_text">
                  {isGenerating?.blurTarget === "face" ? "모든 얼굴" : "등록 인물 제외"}
                </div>
              </div>
              <div className="info_normal_div">
                <div className="info_title">처리 방식</div>
                <div className="info_text">
                  {isGenerating?.processType === "mosaic" ? "블러" : "이모지"}
                </div>
              </div>
              <div className="info_bottom_div">
                <div className="info_title">완료 시간</div>
                <div className="info_text">{completionTime}</div>
              </div>
            </div>
          </div>
        </div>
